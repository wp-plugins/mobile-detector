...
class MobileDTS {
    ...
    private static function detect_device() {
        ...
        foreach (self::$regexps as $group) {
            foreach ($group as $name => $regex) {
                // Initialize dynamic vars to false
                self::$dvars[$name] = false;
                // If match, set to true and check next group
    ...
    public static function is($what) {
        return array_key_exists($what, self::$dvars) ? self::$dvars[$what] : null;
    }
    ...
    private static function switch_theme() {
        $options = get_option('tubal_mobiledts');
